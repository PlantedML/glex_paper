...
# Plot SHAP
df <- rbind(data.frame(Variable = "x1", x = train$x[, 1], y = res$shap[, "x1"]),
            data.frame(Variable = "x2", x = train$x[, 2], y = res$shap[, "x2"]),
            data.frame(Variable = "race", x = train$x[, 3], y = res$shap[, "race"]))
...
# Plot m
df <- rbind(data.frame(Variable = "x1", x = train$x[, 1], y = res$m[, "x1"]),
            data.frame(Variable = "x2", x = train$x[, 2], y = res$m[, "x2"]),
            data.frame(Variable = "race", x = train$x[, 3], y = res$m[, "race"]))
ggplot(df, aes(x = x, y = y)) +
  facet_wrap(~ Variable) +
  geom_point() +
  #scale_color_gradient2(low = "blue", high = "red", mid = "grey") +
...
pred_full <- predict(xg, train$x)
mean_pred <- mean(pred_full)
m_full <- rowSums(res$m) + mean_pred
plot(m_full, predict(xg, train$x))

# Remove race from model
m_norace <- res$m[, grep("race", colnames(res$m), invert = TRUE)]
m_reduced <- rowSums(m_norace) + mean_pred
plot(m_reduced, predict(xg, train$x))

# Re-fit without race
xg_norace <- xgboost(data = train$x[, -3], label = train$y, params = list(max_depth = 10, eta = .5), nrounds = 10)
pred_norace <- predict(xg_norace, train$x[, -3])

df <- rbind(data.frame(model = "original", race = factor(train$x[, 3]), pred = pred_full),
            data.frame(model = "refit", race = factor(train$x[, 3]), pred = pred_norace),
            data.frame(model = "reduce", race = factor(train$x[, 3]), pred = m_reduced))
