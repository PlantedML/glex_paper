...
df <- foreach(bb = 1:5, .combine = rbind) %:%
  foreach(rr = round(seq(1, 1000, length.out = 10)), .combine = rbind) %dopar%
  compare_fun(b = bb, n = 1000, p = 8, depth = 4, rounds = rr)

# Save
saveRDS(df, "runtime_rounds.Rds")

# Plot
df[, seconds := time/1e9]
df2 <- df[, .(seconds = median(seconds)), by = .(rounds, algo)]
ggplot(df2, aes(x = rounds, y = seconds, col = algo)) +
  geom_point() +
  geom_line() +
  theme_bw()
