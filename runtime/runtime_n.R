...
df <- foreach(bb = 1:5, .combine = rbind) %:%
  foreach(nn = round(seq(10^2, 10^4, length.out = 10)), .combine = rbind) %dopar%
  compare_fun(b = bb, n = nn, p = 8, depth = 4, rounds = 100)

# Save
saveRDS(df, "runtime_n.Rds")

# Plot
df[, seconds := time/1e9]
df2 <- df[, .(seconds = median(seconds)), by = .(n, algo)]
ggplot(df2, aes(x = n, y = seconds, col = algo)) +
  geom_point() +
  geom_line() +
  theme_bw()
