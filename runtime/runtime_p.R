...
df <- foreach(bb = 1:5, .combine = rbind) %:%
  foreach(pp = round(seq(2, 10, length.out = 10)), .combine = rbind) %dopar%
  compare_fun(b = bb, n = 10000, p = pp, depth = 4, rounds = 100)

# Save
saveRDS(df, "runtime_p.Rds")

# Plot
df[, seconds := time/1e9]
df2 <- df[, .(seconds = median(seconds)), by = .(p, algo)]
ggplot(df2, aes(x = p, y = seconds, col = algo)) +
  geom_point() +
  geom_line() +
  theme_bw()
