...
rpf<- function(Y, X, max_interaction=2, ntrees=50, splits=30, split_try=10, t_try=0.4, variables=NULL, min_leaf_size=1, alternative=F, loss="L2", epsilon=0.1, categorical_variables=NULL, delta=0, cores=1){
  ...
  b <- apply(X,2,max)+0.01     ### upper bounds
  ...
  tree_fam <- function(run){
    ...
    if(length(Y)==length(X)) dim(X)=c(n,1)
    ...
    return(list(intervals=intervals, values=values, variables=variables, b=b, subsample=subsample, categorical_variables=categorical_variables, individuals=individuals, a=a))
  }
